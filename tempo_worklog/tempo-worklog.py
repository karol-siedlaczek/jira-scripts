#!/usr/bin/env python3
# Copyright 2021 Redge Technologies
# Author: K. Siedlaczek
...
def get_tempo_data(file, include_labels):  # find elements in tree and deletes .xml file
    ...
    for worklog in root:
        ...
        # month = datetime.strptime(work_date, '%Y-%m-%d').strftime('%B') //Tomek chcial te kolumna pustą
        if re.search('.', hours):
            hours = hours.replace('.', ',')
